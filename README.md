...
> ⚠️ **重要限制：只能备份正在运行的容器**  
> 脚本通过 `docker ps` 扫描当前处于 **Running** 状态的容器进行备份。已停止 (Exited) 或暂停 (Paused) 的容器**不会被备份**。原因是：
> - 停止状态的容器没有活跃进程和数据流，备份其运行状态无实际意义
> - 停止的容器通常已不再使用，或处于维护/调试阶段，备份反而会产生无用数据
> - 如果你需要备份某个已停止的容器，请先 `docker start <容器>` 启动它，再执行备份

> 🚨 **数据库备份警告**  
> 目前备份方式为**直接打包卷数据（tar）**，不会对数据库容器（MySQL / PostgreSQL / MongoDB 等）执行 `mysqldump`、`pg_dump` 等专用导出命令。如果在备份过程中数据库正在写入，打包出来的数据文件可能存在不一致风险。
> - **建议将备份定时设在凌晨低峰期**，减少写入冲突
> - MySQL 8.0+ / PostgreSQL 自带 WAL 日志，恢复后会自动回放修复大部分情况
> - 后续版本会加入数据库自动检测 + 专用导出功能，欢迎提 PR
